...
def main(args):
    ...
    model.eval()
    input_ids = tokenizer.encode(args.sentence, add_special_tokens=True,max_length=MAX_LEN,pad_to_max_length=True)
    clean_logit = model(input_ids=torch.LongTensor(input_ids).unsqueeze(0).cuda())[0].cpu()
    print(clean_logit)
